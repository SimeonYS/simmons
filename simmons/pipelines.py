...
class SimmonsPipeline:

    # Database setup
    conn = sqlite3.connect('simmons.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `simmons`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM simmons WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO simmons (date, title, link, content)"
